...
<script>
    $(document).ready(function(){
        ...
        if ($("#home").hasClass("index")) {
            $( "#bodyWrapper" ).removeClass( "container" );
        }else{
            $( ".body-sans-header" ).css("margin-top","15px");
        }

        var pathname = $(location).attr('pathname'); //window.location.pathname; // Returns path only
        var url      = window.location.href;     // Returns full URL
        console.log(pathname, url);
        //$( "#bodyWrapper" ).removeClass( "container" ).addClass( "yourClass" );
