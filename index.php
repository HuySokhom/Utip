...
?>
    ...
    <div class="row text-center"
      style="
          background-image: url('images/site/background-product.jpg');
      ...
      "
    >
      <h1>Welcome to the UTIP Shield Shop</h1>
      <h3>Enjoy living a healthier livestyle</h3>
    </div>
...
<?php
      $category_query = tep_db_query("
        select
          distinct cd.categories_name, c.categories_id
        from
          " . TABLE_CATEGORIES . " c inner join " . TABLE_CATEGORIES_DESCRIPTION . " cd
            on
        ...
        where
          cd.language_id = '" . (int)$languages_id . "'
            order by
          c.sort_order asc ");
      $num_new_category = tep_db_num_rows($category_query);
      if ($num_new_category > 0) {

      $new_cats_content = NULL;
      while ($new_cat = tep_db_fetch_array($category_query)) {
      echo '<div class="container"><h1  style="font-size: 50px;text-align: center;">' . $new_cat['categories_name'] . '</h1>';

      $new_products_query = tep_db_query("
          select
            distinct p.products_id, p.products_image, p.products_tax_class_id, pd.products_name,
            if(s.status, s.specials_new_products_price, p.products_price) as products_price
          from
            " . TABLE_PRODUCTS . " p left join " . TABLE_SPECIALS . " s
              on
          ...
          where
            ...
            p2c.categories_id = c.categories_id
              and
            c.categories_id = " . (int)$new_cat['categories_id'] . "
              and
            p.products_status = '1'
              and
      ...
      if ($num_new_products > 0) {
        ...
        while ($new_products = tep_db_fetch_array($new_products_query)) {
          ...
          '    <div class="caption">' .
          '      <h2 class="text-center">' . $new_products['products_name'] . '</h2></div>' .
          ...
          '      <hr>'.
          '      <p class="text-center">' . $currencies->display_price($new_products['products_price'], tep_get_tax_rate($new_products['products_tax_class_id'])) . '</p>'.
          '      <div class="text-center">'.
          '        <div class="btn-group">'.
          '          <a href="' . tep_href_link(FILENAME_PRODUCT_INFO, tep_get_all_get_params(array('action')) . 'products_id=' . $new_products['products_id']) . '" class="btn btn-default" role="button">' . SMALL_IMAGE_BUTTON_VIEW . '</a>'.
          '          <a href="' . tep_href_link($PHP_SELF, tep_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $new_products['products_id']) . '" class="btn btn-success" role="button">' . SMALL_IMAGE_BUTTON_BUY . '</a>'.
          '        </div>'.
          '      </div><br/>'.
          '  </div>'.
          '</div>';
